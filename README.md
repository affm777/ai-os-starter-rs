...
```bash
git clone https://github.com/affm777/ai-os-starter-rs.git ~/ai-os-starter-rs
cd ~/ai-os-starter-rs
```

> **WICHTIG:** Immer via `git clone` klonen, niemals ZIP-Download. macOS Gatekeeper kann ZIPs sonst blockieren (Quarantine-Attribut). Falls doch ZIP: `xattr -dr com.apple.quarantine ./ai-os-starter-rs`
...
- **Routines anlegen:** damit dein Second Brain sich selbst pflegt → [docs/routines.md](docs/routines.md)
...
│   ├── rules/                # 4 Rule-Files
│   ├── agents/               # 1 Agent (prompt-engineer, interner Worker des /research-prompt-Skills)
│   ├── commands/brain/       # 4 Slash-Commands (Vault-Tooling, Doppelpunkt-Namespace /brain:*)
│   ├── skills/               # 7 Skills (skill-creator, wrap-up, resume-session, new-project, designer, landing-page-builder, research-prompt) — auto-invokable
│   ├── templates/            # Projekt-Templates (new-project: CLAUDE.md + STATE.md)
│   └── hooks/                # 7 Hook-Scripts
│
│  GSD-Commands (/gsd-*) und die gsd-*-Agenten werden separat im Bootstrap via
...
└── docs/
    ...
    ├── google-workspace-cli.md   # gws-CLI + 53 Workspace-Skills
